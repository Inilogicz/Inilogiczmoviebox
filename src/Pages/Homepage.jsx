...
const Homepage = () => {
  ...
  useEffect(() => {
    const getMovies = async () => {
      try {
        ...
        // Set the hero movie
        const top10Movies = responseData.results.slice(10, 20);
  ...
  const handleSearch = async (query) => {
    try {
      const response = await fetch(
        `https://api.themoviedb.org/3/search/movie?api_key=51622a92f8effebccf78ad57f65dc592&query=${encodeURIComponent(query)}`
      );
      const responseData = await response.json();
